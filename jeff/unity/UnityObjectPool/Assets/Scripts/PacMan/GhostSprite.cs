...
public class GhostSprite : MonoBehaviour {
	...
	void Start () {
        this.SetupGhost();
        this.PacManPlayer = PacMan.GetComponent<Player>();
    }
	...
	void OnTriggerEnter2D(Collider2D coll) {
        if (coll.gameObject.tag == "Player")
        {
            ...
            {
				if (DebugText) Debug.Log(string.Format("{0} triggerEnter with {1} change to dead", this, coll.ToString()));
    ...
    public void SetupGhost()
    {
        ...
        this.State = this.ghost.State = GhostState.Roving;
		...
		switch(this.State)
		{
			...
			case GhostState.Roving :
				...
				if(hit[i].rigidbody != null)
				{
					...
					{
							if (DebugText) Debug.Log(string.Format("{0} saw {1} changed to chasing", this, hit[i].rigidbody.tag));
